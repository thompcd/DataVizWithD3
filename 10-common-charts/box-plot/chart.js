async function drawBars() {
  ...
  const rangeLines = binGroups.append("line")
      .attr("class", "line")
      .attr("x1", d => xScale(d.x0) + barWidth / 2)
      .attr("x2", d => xScale(d.x0) + barWidth / 2)
  ...
  const barRects = binGroups.append("rect")
    ...
    const medians = binGroups.append("line")
      ...
      .attr("x2", d => xScale(d.x0) + barPadding / 2 + barWidth - barPadding)
      .attr("y1", d => yScale(d[0].median))
      .attr("y2", d => yScale(d[0].median))

    const rangeMins = binGroups.append("line")
      .attr("class", "line")
      .attr("x1", d => xScale(d.x0) + barPadding / 2 + barWidth * 0.3)
      .attr("x2", d => xScale(d.x1) - barPadding / 2 - barWidth * 0.3)
      .attr("y1", d => yScale(d[0].rangeMin))
    ...
    const outliers = binGroups.append("g")
        .attr("transform", d => `translate(${xScale(d.x0) + barWidth / 2}, 0)`)
      .selectAll("circle")
      .data(d => d[0].outliers)
      .enter().append("circle")
        .attr("class", "outlier")
    ...
    const labels = binGroups.append("text")
        .attr("class", "label")
        .attr("transform", d => `translate(${xScale(d.x0) + barWidth / 2}, -15)`)
